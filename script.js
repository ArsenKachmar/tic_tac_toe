...
let moves = 0, winner = 0, x = 1, o = 3, player = x, computer = o, score = {
    ties: 0,
    player: 0,
    computer: 0,
},
xText = '<span class=\"x\"></span>',
oText = '<span class=\"o\"></span>',
playerText = xText,
computerText = oText,
difficulty = 1,
myGrid = null,
...
Grid.prototype.getRowValues = function(index){
    if(index !== 0 && index !== 1 && index !== 2){
        console.log("Не правильний аргумент в getRowValues");
        return undefined;
    }
    let i = index * 3 ;
...
Grid.prototype.getRowIndices = function(index){
    if(index !== 0 && index !== 1 && index !== 2){
        console.log("Не правильний аргумент в getRowIndices");
        return undefined;
    }
    let row = [];
    index = index * 3;
...
Grid.prototype.getColumnValues = function(index){
    if(index !== 0 && index !== 1 && index !== 2){
        console.log("Не правильний аргумент в getColumnValue");
        return undefined;
    }
    let i, column = [];
...
Grid.prototype.getColumnIndices = function(index){
    if(index !== 0 && index !== 1 && index !== 2){
        console.log("Не правильний аргумент в getColunmIndices");
        return undefined;
    }
    let i, column = [];
    for(i = 0; i < this.getColumnIndices.length; i += 3){
        column.push(i);
    }
    return column;
}
...
Grid.prototype.getDiagValues = function(arg){
    let cells = [];
    if(arg !== 1 && arg !== 0){
        console.log("Помилка arg getDiagValues");
        return undefined;
    } else if (arg === 0){
...
Grid.prototype.getDiagIndices = function(arg){
    if(arg !== 1 && arg !== 0){
        console.log("Помилка arg getDiagIndices");
        return undefined;
    } else if(arg === 0){
        return[0, 4, 8];
    } else {
...
function announceWinner(text) {
  ...
  function endGame(who) {
    if(who == player){
        announceWinner("Вітання ви виграли!")
    } else if (who == computer){
        announceWinner("Переміг комп'ютер");
    } else{
        announceWinner("Нічия");
    }
    gameOver = true;
    whoseTurn = 0;
    moves = 0;
